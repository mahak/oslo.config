...
class ExceptionsTestCase(base.BaseTestCase):
    ...
    def test_required_opt_error(self):
        ...
        self.assertEqual(
            'value required for option foo in group [DEFAULT]', msg
        )
...
class HelpTestCase(BaseTestCase):
    ...
    def test_print_strOpt_with_choices_help(self):
        ...
        self.assertIn('-h, --help', f.getvalue())
        self.assertIn(
            'StrOpt with choices. Allowed values: xx, yy, zz', f.getvalue()
        )
        self.assertIn(
            'StrOpt with choices. Allowed values: <None>, yy, zz', f.getvalue()
        )
        self.assertIn(
            "StrOpt with choices. Allowed values: '', yy, zz", f.getvalue()
        )
    ...
    def test_print_help_with_deprecated(self):
        ...
        if sys.version_info >= (3, 13):
            ...
            self.assertIn(
                '--a-bc A_BC, --d-ef A_BC, --d_ef A_BC', f.getvalue()
            )
            self.assertIn(
                '--u-vw U_VW, --x-yz U_VW, --x_yz U_VW', f.getvalue()
            )
...
class FindConfigFilesTestCase(BaseTestCase):
    def test_find_config_files(self):
        ...
        self.useFixture(
            fixtures.MonkeyPatch('os.path.exists', lambda p: p in config_files)
        )
    ...
    def test_find_config_files_snap(self):
        ...
        self.assertEqual(
            cfg.find_config_files(project='blaa'),
            ['/var/snap/nova/common/etc/blaa/blaa.conf'],
        )
    ...
    def test_find_config_files_with_extension(self):
        ...
        self.assertEqual(
            cfg.find_config_files(project='blaa', extension='.json'),
            config_files,
        )
...
class FindConfigDirsTestCase(BaseTestCase):
    def test_find_config_dirs(self):
        config_dirs = [
            os.path.expanduser('~/.blaa/blaa.conf.d'),
            '/etc/foo.conf.d',
        ]

        self.useFixture(fixtures.MonkeyPatch('sys.argv', ['foo']))
        self.useFixture(
            fixtures.MonkeyPatch('os.path.exists', lambda p: p in config_dirs)
        )
    ...
    def test_find_config_dirs_with_extension(self):
        ...
        self.assertEqual(cfg.find_config_dirs(project='blaa'), [])
        self.assertEqual(
            cfg.find_config_dirs(project='blaa', extension='.json.d'),
            config_dirs,
        )
...
class DefaultConfigFilesTestCase(BaseTestCase):
    ...
    def test_do_not_use_default_multi_arg(self):
        ...
        self.conf(
            args=['--config-file', paths[0]], default_config_files=['bar.conf']
        )
...
class DefaultConfigDirsTestCase(BaseTestCase):
    ...
    def test_do_not_use_default_single_arg(self):
        ...
        p = os.path.dirname(paths[0])
        self.conf(
            args=['--config-dir=' + p], default_config_dirs=['bar.conf.d']
        )
    ...
    def test_find_default_config_dir(self):
        ...
        self.useFixture(
            fixtures.MonkeyPatch(
                'oslo_config.cfg.find_config_dirs', lambda project, prog: p
            )
        )
...
class PositionalTestCase(BaseTestCase):
    def _do_pos_test(self, opt_class, default, cli_args, value):
        self.conf.register_cli_opt(
            opt_class('foo', default=default, positional=True, required=False)
        )
    ...
    def test_positional_uri_default(self):
        self._do_pos_test(
            cfg.URIOpt, 'http://example.com', [], 'http://example.com'
        )
    ...
    def test_positional_uri_arg(self):
        self._do_pos_test(
            cfg.URIOpt, None, ['http://example.com'], 'http://example.com'
        )
    ...
    def test_positional_multistr_arg(self):
        self._do_pos_test(
            cfg.MultiStrOpt, None, ['blaa', 'bar'], ['blaa', 'bar']
        )
...
@mock.patch(
    'oslo_log.versionutils.report_deprecated_feature', _fake_deprecated_feature
)
class ConfigFileOptsTestCase(BaseTestCase):
    ...
    def _do_deprecated_test(
        ...
        self.conf.register_opt(
            opt_class('newfoo', deprecated_name=dname, deprecated_group=dgroup)
        )
    ...
    def _do_dname_test_use(self, opt_class, value, result):
        self._do_deprecated_test(
            opt_class, value, result, 'oldfoo', dname='oldfoo'
        )

    def _do_dgroup_test_use(self, opt_class, value, result):
        self._do_deprecated_test(
            opt_class, value, result, 'newfoo', section='old', dgroup='old'
        )
    ...
    def _do_dname_test_ignore(self, opt_class, value, result):
        self._do_deprecated_test(
            opt_class, value, result, 'newfoo', dname='oldfoo'
        )

    def _do_dgroup_test_ignore(self, opt_class, value, result):
        self._do_deprecated_test(
            opt_class, value, result, 'newfoo', section='DEFAULT', dgroup='old'
        )
    ...
    def test_use_deprecated_for_removal_without_reason(self):
        self.conf.register_cli_opt(
            cfg.StrOpt('oldfoo', deprecated_for_removal=True)
        )
    ...
    def test_conf_file_float_default_wrong_type(self):
        self.assertRaises(
            cfg.DefaultValueError, cfg.FloatOpt, 'foo', default='foobar6.66'
        )
    ...
    def test_conf_file_list_default_wrong_type(self):
        self.assertRaises(
            cfg.DefaultValueError, cfg.ListOpt, 'foo', default=25
        )
    ...
    def test_conf_file_list_item_type(self):
        self.conf.register_cli_opt(
            cfg.ListOpt('foo', item_type=types.Integer())
        )
    ...
    def test_conf_file_list_bounds(self):
        self.conf.register_cli_opt(
            cfg.ListOpt(
                'foo', item_type=types.Integer(), default="[1,2]", bounds=True
            )
        )
        self.conf([])
        self.assertEqual([1, 2], self.conf.foo)
    ...
    def test_conf_file_dict_values_override_deprecated(self):
        self.conf.register_cli_opt(
            cfg.DictOpt('foo', deprecated_name='oldfoo')
        )
    ...
    def test_conf_file_dict_use_dname(self):
        self._do_dname_test_use(
            cfg.DictOpt, 'k1:a,k2:b,k3:c', {'k1': 'a', 'k2': 'b', 'k3': 'c'}
        )

    def test_conf_file_dict_use_dgroup(self):
        self._do_dgroup_test_use(
            cfg.DictOpt, 'k1:a,k2:b,k3:c', {'k1': 'a', 'k2': 'b', 'k3': 'c'}
        )
    ...
    def test_conf_file_dict_ignore_dname(self):
        self._do_dname_test_ignore(
            cfg.DictOpt, 'k1:d,k2:e,k3:f', {'k1': 'd', 'k2': 'e', 'k3': 'f'}
        )

    def test_conf_file_dict_ignore_dgroup(self):
        self._do_dgroup_test_ignore(
            cfg.DictOpt, 'k1:d,k2:e,k3:f', {'k1': 'd', 'k2': 'e', 'k3': 'f'}
        )
    ...
    def test_conf_file_port_list_default(self):
        self.conf.register_opt(
            cfg.ListOpt('foo', item_type=types.Port(), default=[55, 77])
        )
    ...
    def test_conf_file_multistr_values_append_deprecated(self):
        self.conf.register_cli_opt(
            cfg.MultiStrOpt('foo', deprecated_name='oldfoo')
        )
    ...
    def test_conf_file_sorted_group(self):
        ...
        for i in range(10):
            group = cfg.OptGroup(f'group{i}', 'options')
...
class ConfigFileReloadTestCase(BaseTestCase):
    ...
    def test_conf_files_reload_default(self):
        ...
        paths_change = self.create_tempfiles(
            [
...
class ConfigFileMutateTestCase(BaseTestCase):
    ...
    def test_conf_files_mutate_group(self):
        ...
        self.conf.register_cli_opt(
            cfg.StrOpt('boo', mutable=True), group=self.my_group
        )
        self._test_conf_files_mutate()
...
class OptGroupsTestCase(BaseTestCase):
    ...
    def test_arg_group_in_config_file_with_deprecated_name(self):
        self.conf.register_group(cfg.OptGroup('blaa'))
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_name='oldfoo'), group='blaa'
        )
    ...
    def test_arg_group_in_config_file_with_deprecated_group(self):
        self.conf.register_group(cfg.OptGroup('blaa'))
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_group='DEFAULT'), group='blaa'
        )
    ...
    def test_arg_group_in_config_file_with_deprecated_group_and_name(self):
        ...
        self.conf.register_opt(
            cfg.StrOpt(
                'foo', deprecated_group='DEFAULT', deprecated_name='oldfoo'
            ),
            group='blaa',
        )
...
class OptNameSeparatorTestCase(BaseTestCase):
    ...
    def test_deprecated_name(self):
        self.conf.register_opt(
            cfg.StrOpt('foobar', deprecated_name=self.opt_name)
        )
    ...
    def test_deprecated_name_cli(self):
        self.conf.register_cli_opt(
            cfg.BoolOpt('foobar', deprecated_name=self.opt_name)
        )
    ...
    def test_deprecated_opts_cli(self):
        oldopts = [cfg.DeprecatedOpt(self.opt_name)]
        self.conf.register_cli_opt(
            cfg.BoolOpt('foobar', deprecated_opts=oldopts)
        )
...
class TemplateSubstitutionTestCase(BaseTestCase):
    ...
    def test_sub_group_from_default_deprecated(self):
        self.conf.register_group(cfg.OptGroup('ba'))
        self.conf.register_cli_opt(
            cfg.StrOpt('foo', default='123', deprecated_group='DEFAULT'),
            group='ba',
        )
        self.conf.register_cli_opt(cfg.IntOpt('r', default='$foo'), group='ba')
...
class ConfigDirTestCase(BaseTestCase):
    ...
    def test_config_dir_multistr(self):
        ...
        paths = self.create_tempfiles(
            [
                (os.path.join(dir, '00-test'), '[DEFAULT]\nfoo = bar-00\n'),
    ...
    def test_config_dir_default_file_precedence(self):
        ...
        self.conf(
            ['--foo', 'bar', '--config-dir', os.path.dirname(paths[0])],
    ...
    def test_config_dir_doesnt_exist(self):
        ...
        self.assertRaises(
            cfg.ConfigDirNotFoundError, self.conf, ['--config-dir', tmpdir]
        )
...
class OverridesTestCase(BaseTestCase):
    ...
    def test_set_default_not_in_choices(self):
        ...
        self.conf.register_cli_opt(
            cfg.StrOpt('oo', choices=('a', 'b')), group='f'
        )
    ...
    def test_wrong_type_default_override(self):
        ...
        self.assertRaises(
            ValueError, self.conf.set_default, 'foo', 'not_really_a_int'
        )
    ...
    def test__wrong_type_override(self):
        self.conf.register_opt(cfg.IntOpt('foo'))
        self.assertRaises(
            ValueError, self.conf.set_override, 'foo', "not_really_a_int"
        )
...
class UnregisterOptTestCase(BaseTestCase):
    def test_unregister_opt(self):
        ...
        self.assertRaises(
            cfg.ArgsAlreadyParsedError, self.conf.unregister_opt, opts[1]
        )
...
class ImportOptTestCase(BaseTestCase):
    ...
    def test_import_opt_in_group(self):
        ...
        cfg.CONF.import_opt(
            'foo', 'oslo_config.tests.testmods.bar_foo_opt', group='bar'
        )
    ...
    def test_import_opt_import_errror(self):
        self.assertRaises(
            ...
            cfg.CONF.import_opt,
            'blaa',
            'oslo_config.tests.testmods.blaablaa_opt',
        )
    ...
    def test_import_opt_no_such_opt(self):
        self.assertRaises(
            ...
            cfg.CONF.import_opt,
            'blaablaa',
            'oslo_config.tests.testmods.blaa_opt',
        )
...
class ImportGroupTestCase(BaseTestCase):
    ...
    def test_import_group_import_error(self):
        self.assertRaises(
            ...
            'qux',
            'oslo_config.tests.testmods.bazzz_quxxx_opt',
        )
    ...
    def test_import_group_no_such_group(self):
        self.assertRaises(
            ...
            cfg.CONF.import_group,
            'quxxx',
            'oslo_config.tests.testmods.baz_qux_opt',
        )
...
class SadPathTestCase(BaseTestCase):
    ...
    def test_conf_file_not_found(self):
        ...
        self.assertRaises(
            cfg.ConfigFilesNotFoundError, self.conf, ['--config-file', path]
        )

    @unittest.skipIf(
        os.getuid() == 0, "Not supported with the root privileges"
    )
    ...
    def test_conf_file_broken(self):
        ...
        self.assertRaises(
            cfg.ConfigFileParseError, self.conf, ['--config-file', paths[0]]
        )
...
class FindFileTestCase(BaseTestCase):
    def test_find_file_without_init(self):
        self.assertRaises(
            cfg.NotInitializedError, self.conf.find_file, 'foo.json'
        )
    ...
    def test_find_policy_file(self):
        ...
        self.useFixture(
            fixtures.MonkeyPatch('os.path.exists', lambda p: p == policy_file)
        )
    ...
    def test_find_policy_file_with_config_file(self):
        ...
        paths = self.create_tempfiles(
            [
                (os.path.join(dir, 'test.conf'), '[DEFAULT]'),
    ...
    def test_find_policy_file_with_config_dir(self):
        ...
        path = self.create_tempfiles(
            [(os.path.join(dir, 'policy.json'), '{}')], ext=''
        )[0]
...
class ConfigParserTestCase(BaseTestCase):
    ...
    def test__parse_file_ioerror(self):
        ...
        with mock.patch('oslo_config.cfg.ConfigParser.parse') as parse:
            parse.side_effect = OSError(
                errno.EMFILE, filename, 'Too many open files'
            )
            self.assertRaises(
                IOError, cfg.ConfigParser._parse_file, filename, namespace
            )
...
class NamespaceTestCase(BaseTestCase):
    ...
    def assertAbsent(self, key, normalized=False):
        self.assertRaises(
            KeyError, self.ns._get_value, [key], normalized=normalized
        )

    def assertValue(self, key, expect, multi=False, normalized=False):
        actual, _ = self.ns._get_value(
            [key], multi=multi, normalized=normalized
        )
    ...
    def test_single_file(self):
        ...
        self.assertValue(
            ('DEFAULT', 'foo'), ['bar'], multi=True, normalized=True
        )

        self.assertValue(('BLAA', 'bar'), 'foo')
        self.assertValue(('BLAA', 'bar'), ['foo'], multi=True)
        self.assertValue(('blaa', 'bar'), ['foo'], multi=True, normalized=True)
    ...
    def test_attrs_subparser(self):
        ...
        CONF.register_cli_opt(
            cfg.SubCommandOpt('foo', handler=lambda sub: sub.add_parser('foo'))
        )
        CONF(['foo'])
...
class TildeExpansionTestCase(BaseTestCase):
    def test_config_file_tilde(self):
        ...
        self.useFixture(
            fixtures.MonkeyPatch('os.path.exists', lambda p: p == tmpfile)
        )
    ...
    def test_config_dir_tilde(self):
        ...
        try:
            ...
            self.useFixture(
                fixtures.MonkeyPatch('glob.glob', lambda p: [tmpfile])
            )

            e = self.assertRaises(
                cfg.ConfigFilesNotFoundError,
                self.conf,
                ['--config-dir', os.path.join('~', os.path.basename(tmpdir))],
            )
...
class MultipleDeprecatedOptionsTestCase(BaseTestCase):
    def test_conf_file_override_use_deprecated_name_and_group(self):
        ...
        self.conf.register_opt(
            cfg.StrOpt(
                'foo', deprecated_name='oldfoo', deprecated_group='oldgroup'
            ),
            group='blaa',
        )
    ...
    def test_conf_file_override_use_deprecated_opts(self):
        ...
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_opts=oldopts), group='blaa'
        )
    ...
    def test_conf_file_override_use_deprecated_multi_opts(self):
        self.conf.register_group(cfg.OptGroup('blaa'))
        oldopts = [
            cfg.DeprecatedOpt('oldfoo', group='oldgroup'),
...
class MultipleDeprecatedCliOptionsTestCase(BaseTestCase):
    ...
    def test_conf_file_common_deprecated_group(self):
        ...
        self.conf.register_opt(
            cfg.StrOpt('common_opt', deprecated_opts=oldopts), group='bar'
        )
        self.conf.register_opt(
            cfg.StrOpt('common_opt', deprecated_opts=oldopts), group='foo'
        )
...
class ChoicesTestCase(BaseTestCase):
    def test_choice_default(self):
        self.conf.register_cli_opt(
            cfg.StrOpt(
                'protocol', default='http', choices=['http', 'https', 'ftp']
            )
        )
        self.conf([])
    ...
    def test_conf_file_choice_none_value(self):
        self.conf.register_opt(
            cfg.StrOpt('foo', default=None, choices=[None, 'bar1', 'bar2'])
        )
    ...
    def test_conf_file_choice_value_override(self):
        self.conf.register_cli_opt(
            cfg.StrOpt('foo', choices=['baar', 'baaar'])
        )
...
class PortChoicesTestCase(BaseTestCase):
    def test_choice_default(self):
        self.conf.register_cli_opt(
            cfg.PortOpt('port', default=455, choices=[80, 455])
        )
        self.conf([])
...
class RegexTestCase(BaseTestCase):
    ...
    def test_conf_file_regex_bad_value(self):
        ...
        self.assertRaisesRegex(
            ValueError, "doesn't match regex", getattr, self.conf, 'foo'
        )

    def test_regex_with_choice(self):
        self.assertRaises(
            ValueError, cfg.StrOpt, 'foo', choices=['bar1'], regex='bar2'
        )
...
class QuotesTestCase(BaseTestCase):
    ...
    def test_conf_file_quotes_good_value(self):
        self.conf.register_opt(cfg.StrOpt('foo', quotes=True))
    ...
    def test_conf_file_quotes_bad_value(self):
        ...
        self.assertRaisesRegex(
            ValueError, 'Non-closed quote:', getattr, self.conf, 'foo'
        )
...
class IgnoreCaseTestCase(BaseTestCase):
    def test_ignore_case_with_choices(self):
        self.conf.register_cli_opt(
            cfg.StrOpt(
                'foo', ignore_case=True, choices=['bar1', 'bar2', 'BAR3']
            )
        )
        self.conf(['--foo', 'bAr1'])
        self.assertEqual('bAr1', self.conf.foo)
    ...
    def test_ignore_case_with_regex(self):
        self.conf.register_cli_opt(
            cfg.StrOpt('foo', ignore_case=True, regex='fOO|bar')
        )
        self.conf(['--foo', 'foo'])
        self.assertEqual('foo', self.conf.foo)
        self.conf(['--foo', 'Bar'])
    ...
    def test_conf_file_ignore_case_with_regex(self):
        self.conf.register_opt(
            cfg.StrOpt('foo', ignore_case=True, regex='bAr')
        )
...
class URIOptMaxLengthTestCase(BaseTestCase):
    ...
    def test_uriopt_max_length_bad(self):
        self.conf.register_cli_opt(cfg.URIOpt('foo', max_length=30))
        self.assertRaises(
            SystemExit, self.conf, ['--foo', 'http://www.example.com/versions']
        )
...
class URIOptSchemesTestCase(BaseTestCase):
    ...
    def test_uriopt_schemes_bad(self):
        self.conf.register_cli_opt(cfg.URIOpt('foo', schemes=['http', 'ftp']))
        self.assertRaises(
            SystemExit, self.conf, ['--foo', 'https://www.example.com']
        )
        self.assertRaises(
            SystemExit, self.conf, ['--foo', 'file://www.example.com']
        )
...
class SectionsTestCase(BaseTestCase):
    ...
    def test_list_all_sections_post_mutate(self):
        ...
        self.conf.mutate_config_files()
        self.assertEqual(
            ['BLAA', 'DEFAULT', 'WOMBAT'], self.conf.list_all_sections()
        )
...
class DeprecationWarningTestScenarios(DeprecationWarningTestBase):
    scenarios = [
        ('default-deprecated', dict(deprecated=True, group='DEFAULT')),
        ('default-not-deprecated', dict(deprecated=False, group='DEFAULT')),
    ...
    def test_deprecated_logging(self):
        ...
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_name='bar'), group='other'
        )
        if self.deprecated:
            content = 'bar=baz'
        else:
...
class DeprecationWarningTests(DeprecationWarningTestBase):
    ...
    def test_DeprecatedOpt(self):
        ...
        self.conf.register_group(cfg.OptGroup('other'))
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_opts=default_deprecated)
        )
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_opts=other_deprecated), group='other'
        )
        paths = self.create_tempfiles(
            [('test', '[DEFAULT]\n' + 'bar=baz\n' + '[other]\n' + 'baz=baz\n')]
        )
    ...
    def assert_message_logged(
        self, deprecated_name, deprecated_group, current_name, current_group
    ):
    ...
    def test_deprecated_for_removal(self):
        ...
        expected = (
            'Option "foo" from group "DEFAULT" is '
    ...
    def test_deprecated_for_removal_with_group(self):
        self.conf.register_group(cfg.OptGroup('other'))
        self.conf.register_opt(
            cfg.StrOpt('foo', deprecated_for_removal=True), group='other'
        )
        self.conf.register_opt(
            cfg.StrOpt('bar', deprecated_for_removal=True), group='other'
        )
        ...
        expected = (
            'Option "foo" from group "other" is '
            'deprecated for removal.  Its value may be silently '
            'ignored in the future.\n'
        )
        self.assertIn(expected, self.fake_logger.output)
    ...
    def test_deprecated_with_dest(self):
        self.conf.register_group(cfg.OptGroup('other'))
        self.conf.register_opt(
            cfg.StrOpt('foo-bar', deprecated_name='bar', dest='foo'),
            group='other',
        )
        content = 'bar=baz'
        paths = self.create_tempfiles([('test', '[other]\n' + content + '\n')])
